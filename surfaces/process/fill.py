...
'''for i in range(number_of_ellipsoids):
    ...
    if (abs(rhos[i]) >= abs(b)):
        bpy.ops.mesh.primitive_plane_add(location=center_char_circles[i])
...
for i in range(half + 1):         
    if (abs(rhos[half + i]) >= abs(b)):
        bpy.ops.mesh.primitive_circle_add(location=center_char_circles[half+i])
        circle = bpy.context.object
        circle.name = "Circle"
...
current_state = bpy.data.objects["Circle"].select_get()
bpy.context.view_layer.objects.active = bpy.data.objects['Circle']'''

'''objectToSelect = bpy.data.objects["Circle"]
...
for i in range(number_of_ellipsoids):
    if (abs(rhos[i]) < abs(b)):
        ...
        '''bpy.ops.mesh.primitive_circle_add(location=center_char_circles[i]+rhos[i]*normal_vectors_of_plane[i])
