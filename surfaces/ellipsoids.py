...
for t_value in t_values:
    ...
    if (normal_vector_of_plane != sp.Matrix([0, 0, 0])):
        normal_vector_of_plane_at_point = normal_vector_of_plane.subs(t, t_value)
    else: 
        normal_vector_of_plane_at_point = sp.Matrix([0, 0, 0])
...
original_normal = sp.Matrix([0, 0, 1])
for i in range(number_of_ellipsoids): 
    if (rhos[i] <= b):
        bpy.ops.mesh.primitive_circle_add(scale=(b, a), location=center_char_circles[i]+rhos[i]*normal_vectors_of_plane[i])
        ellipse_object = bpy.context.object
        rotated_normal_vector = normal_vectors_of_plane[i]
        # Calculate rotation
        rotation_angle, rotation_axis = en.find_rotation2(original_normal, rotated_normal_vector)
        # Apply the rotation
        ellipse_object.rotation_mode = 'AXIS_ANGLE'
        ellipse_object.rotation_axis_angle[0] = rotation_angle
        ellipse_object.rotation_axis_angle[1:] = rotation_axis
